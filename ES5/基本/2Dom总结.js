...
/*
	...
	节点(相关信息,非自身)
			...
			节点类型
					标签:1 dom.nodeType
					属性:2 dom.childNodes[0].nodeType
					文本:3 空格也是
					注释:8
					#document:9  
			获取元素位子信息
					dom.getBoundingClientRect() 盒子信息，不带单位
							width  带边框，下面计算都带
							height 带边框
							left   元素左边到可视区域最左边
							right  元素右边到可视区域最左边
	...
	节点(自身信息，增删改查)	
			...
			增加节点(操作已有的则是剪贴)
				...
				replacechild替换元素用法相通,被替换的会被删除
			删除节点
				父节点.removeChild(节点) 不加引号
